        ...
        <div className="image flex flex-col md:flex-row gap-4 md:gap-8 justify-between">
          <div className="w-full flex flex-row justify-between md:flex-col md:w-[100px] order-2 md:order-1">
            {product?.images.slice(0,4).map((img, index) => (
              <div key={index} className="img mb-3 border border-main-color rounded-3xl cursor-pointer overflow-hidden">
                <img src={img} className="w-[80px]" alt="product-image" />
              </div>
            ))}
          </div>
          <div className="h-[400px] md:h-[500px] w-full relative order-1 md:order-2">
            <img src={product?.imageCover} className="w-full h-full" alt="product-image" />
            <span className="absolute top-5 right-5 w-10 h-10 rounded-full text-main-color text-lg bg-white border border-main-color flex justify-center items-center">
